...
export class EditorHelper {
  ...
  private updateSegMapping(value, key, updateTrace = false) {
    if (REMOVE_KEY === value) {
      ...
      console.log("inside update seg print data" + JSON.stringify(s))
      const opts: any = {
        value: value,
        series: s,
      };
    ...
    if (updateTrace) {
      ...
      console.log('SET in updatetrace', key, value, this.trace.mapping);
    }
  }
  ...
  onAxisSeriesChanged(axis: AxisInfo) {
    console.log("mounish in editor.js" + JSON.stringify(axis) + JSON.stringify(axis.segment.value));
