...
class PlotlyPanelCtrl extends MetricsPanelCtrl {
  ...
  getProcessedLayout() {
    ...
    if (this.is3d() && (this.cfg.settings.type !== "heatmap")) {
      if (!layout.zaxis) {
        layout.zaxis = {};
      }
      ...
      delete layout.zaxis;
      delete layout.scene;
      console.log("Im in heatmap non 3d");
  ...
  onRender() {
    ...
    if (!this.initialized) {
      ...
      console.log("is this after that getProcessedLayout");
      console.log("layout details" + JSON.stringify(this.layout));
  ...
  onDataReceived(dataList) {
    console.log("mounish ondatareceived" + JSON.stringify(dataList));
    ...
    if (dataList && dataList.length > 0) {
      ...
      dataList.forEach((series, sidx) => {
        ...
        console.log("inside datalist for loop" + JSON.stringify(series) + "sidx " + JSON.stringify(sidx));
        if (series.columns) {
  ...
  __addCopyPath(trace: any, key: string, path: string) {
    if (key) {
      ...
      console.log("in addcopypath" + JSON.stringify(s));
      if (!s) {
    ...
    this.traces = this.cfg.traces.map((tconfig, idx) => {
      ...
      if (config.show.markers) {
        ...
        if (config.settings.color_option === 'ramp') {
          ...
          //console.log(mapping.color + "mounish ");
        } else {
  ...
  _updateTraceData(force = false): boolean {
    ...
    if (force || !this.traces) {
      ...
      console.log("uptracedata in force true if");
    ...
    this.traces.forEach(trace => {
      if (trace.__set) {
        trace.__set.forEach(v => {
          ...
          console.log("s values "+ JSON.stringify(s));
          let vals: any[] = zero;

          if (s) {
            if ((this.cfg.settings.type !== 'heatmap') || ((this.cfg.settings.type === 'heatmap') && (v.path !== "z"))) {
              vals = s.toArray();
              console.log("updttracedata missing vals" +"key is "+JSON.stringify(v)+" "+ JSON.stringify(vals));
          ...
          if (!vals) {
            console.log("no vals " + v.path +" end");
            vals = zero;
          }
          if((this.cfg.settings.type === 'heatmap') && (v.path === "z")) {
            try{
              _.set(trace, v.path, s["table"]["rows"]);
              console.log("try stmt in table");
            }catch(ex){
              console.log("try stmt in series");
              _.set(trace, v.path, s["series"]["datapoints"]);
            }
            /*
            var arr2 : any[] = [];
            for (var var1 in s) {
              if ((var1 == "table") || (var1 == 'series')){
                for (var var2 in s[var1]) {
                  if ((var2 == "rows") ||(var2 == 'datapoints')) {
                    arr2 = s[var1][var2];
                    console.log("happy "+ JSON.stringify(arr2));
                  }
                }
              }
              console.log("properties" + JSON.stringify(s[var1]));
            }
            _.set(trace, v.path, s["table"]["rows"]);
            console.log("this shld work" + JSON.stringify(s));
            */
          }else {
  ...
  onConfigChanged() {
    ...
    loadIfNecessary(this.cfg).then(res => {
      ...
      if (this.initialized && this.graphDiv) {
        ...
        console.log('mounish' + JSON.stringify(this.layout) + JSON.stringify(this.cfg));
        Plotly.react(this.graphDiv, traces, this.layout, options);
      }
  ...
  is3d() {
    if (this.cfg.settings.type === 'scatter3d') {
      return true
    } else if (this.cfg.settings.type === 'heatmap') {
      return true
    } else {
      return false
    }
